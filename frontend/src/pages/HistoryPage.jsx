...
export default function HistoryPage() {
  ...
  const handlePreview = async (record) => {
    ...
    if (!record.output_text) {
      ...
      try {
        const full = await getReport(record.id);
  ...
  const handleCopy = async () => {
    ...
    const text = previewReport.output_text;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => (
        <Button type="link" style={{ padding: 0 }} onClick={() => handlePreview(record)}>
          {text || '未命名汇报'}
        </Button>
      ),
    },
    ...
    },
    {
      ...
      render: (t) => <span style={{ whiteSpace: 'nowrap' }}>{new Date(t).toLocaleString('zh-CN')}</span>,
    },
    {
